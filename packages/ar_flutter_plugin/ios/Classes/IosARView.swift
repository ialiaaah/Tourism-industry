...
import ARKit
import Combine
// ARCoreCloudAnchors removed — not needed on iOS (ARKit only).
...
class IosARView: NSObject, FlutterPlatformView, ARSCNViewDelegate, UIGestureRecognizerDelegate, ARSessionDelegate {
    ...
    func onAnchorMethodCalled(_ call :FlutterMethodCall, _ result: @escaping FlutterResult) {
        ...
        switch call.method {
            ...
            case "initGoogleCloudAnchorMode":
                // Cloud Anchors not supported — ARCore removed to fix nanopb/Firebase conflict.
                sessionManagerChannel.invokeMethod("onError", arguments: ["Cloud Anchors not available on this build."])
                break
            case "uploadAnchor":
    ...
    func session(_ session: ARSession, didUpdate frame: ARFrame) {
        // ARCore session update removed — iOS uses ARKit only.
    }
    ...
    // Cloud Anchor listeners stubbed — ARCore removed (iOS uses ARKit only).
    private class cloudAnchorUploadedListener: CloudAnchorListener {
        private var parent: IosARView
        init(parent: IosARView) { self.parent = parent }
    ...
    func decodeCloudAnchorState(state: AnyObject?) -> String { return "Not supported" }
}
